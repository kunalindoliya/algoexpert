/**
 * @read: https://leetcode.com/problems/longest-repeating-character-replacement/
 */
...
const characterReplacement = function(s, k) {
    let res = 0;
    let count = new Map(); // maintain the frequency of characters
    let l = 0;

    for (let r = 0; r < s.length; r++) {
        let len  = r - l + 1
        count.set(s[r], 1 + (count.get(s[r]) || 0))
        // only change the left pointer if and only if length of window - max frequency > k
        // because window is valid if the difference is <= k
        if ((len - Math.max(...count.values())) > k) {
            count.set(s[l], count.get(s[l]) - 1) // need to decrease the frequency
            l++;
        }
        len = r - l + 1;
